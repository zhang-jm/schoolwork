...
{ 
  ...
  */
			 ...
			 int speed, Line hLine, Line vLine)
  {
    ...
    this.speed = speed; //current PacMan speed

    //makes MouseEvents work
  ...
  */
  public void run()
  {
    ...
    {
      if(isRunning) //if the stop button is not pressed
      {
        this.startRunning();
      }
      else
      ...
      //checks if the canvas has been resized and moves PacMan based on the
      //ratio of where PacMan is in regards to the canvas
      this.updateLocation();
 
      //used to make PacMan look animated, takes the speed from the slider
      if(speed < MAX_SPEED)
      {
