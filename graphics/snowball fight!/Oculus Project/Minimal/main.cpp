...
glm::vec3 posVector = glm::vec3(0.0f);
...
public:
	ColorCubeScene() : cube({ "Position", "Normal" }, oglplus::shapes::Cube()) {
		...
		floorShader = LoadShaders("./floorshader.vert", "./floorshader.frag");
		...
		ditto = new Player(glm::vec3(-75.0f, -10.0f, -100.0f), true);
	...
	void render(const mat4 & projection, const mat4 & modelview, Line* leftController, Line* rightController) {
		using namespace oglplus;
		glm::vec3 playerSnowballPos;
		glm::vec3 enemySnowballPos;
		glm::vec3 esb;

		client->playerX = posVector.x;
		client->playerY = posVector.y;
		client->playerZ = posVector.z;
		client->playerSnowballInHand = thisPlayer->mySnowballs->holdingSnowball;
		enemyPlayer->move(glm::vec3(-client->enemyPlayerX, client->enemyPlayerY, -client->enemyPlayerZ));
		if(client->playerSnowballInHand) playerSnowballPos = thisPlayer->mySnowballs->snowballInHand->position;
		client->snowballX = playerSnowballPos.x;
		client->snowballY = playerSnowballPos.y;
		client->snowballZ = playerSnowballPos.z;
		if (client->enemySnowballInHand && !enemyPlayer->mySnowballs->holdingSnowball) {
			enemyPlayer->grabSnowball();
		}
		if (enemyPlayer->mySnowballs->holdingSnowball) {
			esb = enemyPlayer->mySnowballs->snowballInHand->position;
			if ((abs(esb.x - thisPlayer->headPosition.x) < 5.0f || abs(esb.y - thisPlayer->headPosition.y) < 5.0f) &&
				(abs(esb.z - thisPlayer->headPosition.z) < 10.0f)) {
				hit = true;
				...
				enemySnowballPos = glm::vec3(-client->enemySnowballX, client->enemySnowballY, -client->enemySnowballZ);
				enemyPlayer->mySnowballs->snowballInHand->updatePosition(enemySnowballPos);
			}
		}		

		if (hit == true && timer < 5.0f) {
			timer += 0.05f;
			glClearColor(1.0f, 0.0f, 0.0f, 1.0f);
		}
		else if (client->enemyHit) {
			timer = 0.0f;
			hit = false;
			...
			floor->draw(floorShader, projection, modelview);
			...
			// Render the cube
			//converted = snowballs->drawParticles(0.05f, shaderProgram, projection, modelview, leftController, rightController);
			//glm::mat4 mv = modelview * glm::translate(glm::mat4(1.0f), glm::vec3(30.0f, 0.0f, 0.0f));
			thisPlayer->draw(shaderProgram, 0.05f, projection, modelview, rightController, true);
			enemyPlayer->draw(shaderProgram, 0.05f, projection, modelview, rightController, true);
			ditto->draw(shaderProgram, 0.05f, projection, modelview, rightController, false);
			timer = 0.0f;
			hit = false;
		}

		client->hitByEnemy = hit;
		char buff[100];

		if (win == true) {
			//sprintf_s(buff, "Win true\n");
			//OutputDebugStringA(buff);
		}
		else {
			//sprintf_s(buff, "Win false\n");
			//OutputDebugStringA(buff);
		}

		if (gameOver == true) {
			//sprintf_s(buff, "GO true\n");
			//OutputDebugStringA(buff);
		}
		else {
			//sprintf_s(buff, "GO false\n");
			//OutputDebugStringA(buff);
		}

		/*if (snowballs->particleCount == 0) {
			//snowballs->removeSnowball();
			win = true;
			gameOver = true;
		}*/
		...
		if (!win) {
			//update (from idle callback)
			if (trackSpawn < 10.0f) {
				trackSpawn += spawnRate;
			}
			else {
				trackSpawn = 0.0f;
				//gameOver = snowballs->spawnSnowball();
			}
		}
...
protected:
	void initGl() override {
		...
		leftController = new Line();
		rightController = new Line();
	...
	void renderScene(const glm::mat4 & projection, const glm::mat4 & headPose) override {
		...
		leftController->setEndPoints(leftHandPosition, glm::toMat4(leftHandOrient));
		...
		rightController->setEndPoints(rightHandPosition, glm::toMat4(rightHandOrient));
		...
		if (rightHandTriggerPressed) {
			rightController->pressed = true;
			rightController->draw(1.0f, lineShader, projection, glm::inverse(headPose));
			//ovr_SetControllerVibration(_session, ovrControllerType_RTouch, 0.0f, 1.0f);
		}
		else {
			rightController->pressed = false;
			//ovr_SetControllerVibration(_session, ovrControllerType_RTouch, 0.0f, 0.0f);
			rightController->draw(0.0f, lineShader, projection, glm::inverse(headPose));
		}

		cubeScene->render(projection, glm::inverse(headPose), leftController, rightController);
